// ☘️ St. Patrick's Day Filter v14
// Clean dome pile inside pot · no debug · smile/teeth trigger
...
// Pile system
// ─────────────────────────────────────────
// Physics: simple integer count of how many coins are stacked per X-slot
// Rendering: dome of coin PNGs, fully clipped to pot opening

const PILE_SLOTS = 24;   // horizontal slots across the pot opening
let   slotCount  = [];   // integer coin count per slot
let   pileAlpha  = 1.0;
...
// Coin size in pile (uniform, ~4% of screen width → looks like reference)
function pCS() { return Math.round(canvas.width * 0.040); }

// Max coins per slot (how tall the dome can get)
function maxCoinsPerSlot() { return 8; }

// Add one coin to the pile slot closest to x
function addCoinToPile(x) {
  const { cx, rimRX } = getPot();
  // Reject if outside pot opening
  if (Math.abs(x - cx) > rimRX * 0.88) return;

  const t    = (x - (cx - rimRX * 0.88)) / (rimRX * 1.76); // 0..1 across opening
  const slot = Math.max(0, Math.min(PILE_SLOTS-1, Math.floor(t * PILE_SLOTS)));

  // Also add to neighbours slightly for dome shape
  const max = maxCoinsPerSlot();
  if (slotCount[slot] < max) slotCount[slot]++;
  if (slot > 0              && slotCount[slot-1] < max) slotCount[slot-1] = Math.max(slotCount[slot-1], slotCount[slot]-1);
  if (slot < PILE_SLOTS-1   && slotCount[slot+1] < max) slotCount[slot+1] = Math.max(slotCount[slot+1], slotCount[slot]-1);
}

// Surface Y for falling-coin collision at canvas x
function surfaceYAt(x) {
  const { cx, rimY, rimRX } = getPot();
  if (Math.abs(x - cx) > rimRX * 0.92) return rimY; // outside pot → use rim as floor
  const t    = (x - (cx - rimRX * 0.88)) / (rimRX * 1.76);
  const slot = Math.max(0, Math.min(PILE_SLOTS-1, Math.floor(t * PILE_SLOTS)));
  const cs   = pCS();
  return rimY - slotCount[slot] * cs * 0.80;
}

// Draw the dome pile — only PNG coins, tight rows, clipped to pot opening
function drawPile() {
  if (slotCount.every(c=>c===0)) return;

  const { cx, rimY, rimRX } = getPot();
  const cs   = pCS();        // coin radius (half-size)
  ...
  // Clip strictly to pot opening width
  ctx.beginPath();
  ...
  for (let s = 0; s < PILE_SLOTS; s++) {
    ...
    for (let row = 0; row < count; row++) {
      // Y: stack upward from rimY, each row offset by ~80% of coin diameter
      const y = rimY - row * cs * 1.62 - cs * 0.6;
      if (y + cs < 0) continue;

      // Tiny deterministic jitter (looks natural, stable each frame)
      const jx = Math.sin(s * 11.3 + row * 7.1) * cs * 0.22;
      const jy = Math.cos(s * 8.7  + row * 4.3) * cs * 0.12;
      const tilt = Math.sin(s * 17.1 + row * 6.3) * 0.20;

      const sz = cs * 2.1; // diameter
      ctx.save();
...
function drawPot() {
  ...
  // Rim shadow
  ctx.beginPath(); ctx.ellipse(cx,rimY+rimRY*.6,rimRX*.98,rimRY*.65,0,0,Math.PI*2);
  ...
  // Inner shadow
  ctx.beginPath(); ctx.ellipse(cx,rimY+rimRY*.28,rimRX*.82,rimRY*.52,0,0,Math.PI*2);
  ctx.fillStyle='rgba(0,0,0,0.60)'; ctx.fill();

  // Shine
  ctx.beginPath(); ctx.ellipse(cx-rimRX*.23,rimY-rimRY*.28,rimRX*.26,rimRY*.30,-0.14,0,Math.PI*2);
...
// ─────────────────────────────────────────
// Falling coins — only spawn inside pot X range so they arc down into the pot
// ─────────────────────────────────────────
...
function spawnFromSky() {
  ...
  for (let i=0; i<3; i++) {
    ...
    // Spawn anywhere across screen width — let them drift and bounce into pot
    falling.push({
...
function stepFalling() {
  ...
  for (let i=0; i<falling.length; i++) {
    ...
    // Coins outside the pot settle at rimY and disappear (no pile there)
    if (Math.abs(p.x-cx)>rimRX*0.92 && p.y+p.size*.5>=rimY) {
      toRemove.push(i); continue;
    }

    // Coins inside the pot settle on the pile surface
    const surf=surfaceYAt(p.x);
    if (p.y+p.size*.5>=surf) {
      addCoinToPile(p.x);
      toRemove.push(i); continue;
    }
